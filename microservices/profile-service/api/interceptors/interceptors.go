...
func authorize(req any) error {
	...
	case *profilev1.MeRequest:
		return requireUser(request.GetUser().GetUserId(), request.GetUser().GetRole())
	case *profilev1.UpdateProfileRequest:
		return requireUser(request.GetUser().GetUserId(), request.GetUser().GetRole())
	case *profilev1.AddWorkerRequest:
		return requireAnyRole(request.GetActor().GetUserId(), request.GetActor().GetRole(), "admin", "manager")
	case *profilev1.ListWorkersRequest:
		return requireAnyRole(request.GetActor().GetUserId(), request.GetActor().GetRole(), "admin", "manager")
	case *profilev1.SetWorkerAvailabilityRequest:
		return requireAnyRole(request.GetWorker().GetUserId(), request.GetWorker().GetRole(), "employee")
	case *profilev1.ListWorkerAvailabilityRequest:
		return requireUser(request.GetActor().GetUserId(), request.GetActor().GetRole())
	default:
